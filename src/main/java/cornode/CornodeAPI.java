...
/**
 * cornodeAPI Builder. Usage:
 * <p>
 * cornodeApiProxy api = cornodeApiProxy.Builder
 * .protocol("http")
 ...
 * @author davassi
 */
public class cornodeAPI extends cornodeAPICore {

    private static final Logger log = LoggerFactory.getLogger(cornodeAPI.class);
    private ICurl customCurl;

    protected cornodeAPI(Builder builder) {
        super(builder);
    ...
    public GetNewAddressResponse getNewAddress(final String seed, int security, final int index, final boolean checksum, final int total, final boolean returnAll) throws InvalidSecurityLevelException, InvalidAddressException {
        ...
        if (total != 0) {
            for (int i = index; i < index + total; i++) {
                allAddresses.add(cornodeAPIUtils.newAddress(seed, security, i, checksum, customCurl.clone()));
            }
        ...
        for (int i = index; ; i++) {
            ...
            final String newAddress = cornodeAPIUtils.newAddress(seed, security, i, checksum, customCurl.clone());
    ...
    public GetBalancesAndFormatResponse getInputs(String seed, int security, int start, int end, long threshold) throws InvalidSecurityLevelException, InvalidAddressException {
        ...
        if (end != 0) {
            ...
            for (int i = start; i < end; i++) {
                ...
                String address = cornodeAPIUtils.newAddress(seed, security, i, false, customCurl.clone());
        ...
        for (int i = 0; i < inputs.size(); i++) {
            ...
            if (thisBalance >= totalTransferValue) {
                ...
                if (remainder > 0 && remainderAddress != null) {
                    ...
                    return cornodeAPIUtils.signInputsAndReturn(seed, inputs, bundle, signatureFragments, customCurl.clone());
                } else if (remainder > 0) {
    ...
    public static class Builder extends cornodeAPICore.Builder<Builder> {
        ...
        public cornodeAPI build() {
            super.build();
            return new cornodeAPI(this);
        }
    }
}
